...
TrajectoryGenerator2DCircle_impl::TrajectoryGenerator2DCircle_impl(
  ...
  rayon = new DoubleSpinBox(reglages_groupbox->LastRowLastCol(), "R", " m", 0,
                            1000, .1);
  veloctity = new DoubleSpinBox(reglages_groupbox->LastRowLastCol(), "toast",
                                " m/s", -10, 10, 1);
  acceleration =
  ...
  if (is_running) {
    if (R == 0) {
      ...
      if (CurrentTime < V / A) {
        ...
        pos.x = R * cos(theta) * 10;
        pos.y = R * sin(theta) * 10;
        v.x = -A * CurrentTime * sin(theta) * 10;
        v.y = A * CurrentTime * cos(theta) * 10;
      } else {
        if (!is_finishing) {
          theta =
